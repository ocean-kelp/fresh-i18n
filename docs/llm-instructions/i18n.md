...
t("common.actions.save"); // ✅ No props needed!

// Islands - OLD WAY (migrate away from this)
const t = translate(translationData, config);
t("common.actions.save"); // ⚠️ Legacy - requires prop-drilling
```
...
app.use(i18nPlugin({
  ...
  defaultLanguage: "en",
  localesDir: "./locales",
  clientLoad: { // 🔑 Enable client-side injection
    always: ["common"],
    routes: {
...
export default function MyIsland() {
  const t = useTranslation(); // ✨ No props needed!
...
⚠️ **This approach is being phased out. Migrate to the client-side injection approach above.**
...
export default function MyComponent({ state }) {
  ...
  return (
    <div>
      ...
      <h1>{tDashboard("title")}</h1> {/* → t("features.dashboard.title") */}
    </div>
  );
}
```
...
1. ✅ Enable `clientLoad` with `fallback: "none"`
2. ✅ Migrate routes one at a time
3. ✅ Test each route before moving to next
4. ✅ Zero duplication during migration
5. ✅ See 90% payload reduction per migrated route
